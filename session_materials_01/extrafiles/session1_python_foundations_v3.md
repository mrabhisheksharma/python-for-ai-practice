...
## 1. Python Core Basics (10 min)
*Optional / audience-dependent — skip or speed through if the group already knows this; cover fully if there's a mix or anyone needs a refresher.*

### 1.1 Variables & Data Types (5 min)
```python
name = "Model-A"        # str
accuracy = 91.4         # float
epochs = 50             # int
is_trained = True       # bool
notes = None             # NoneType — "nothing here yet"

print(type(accuracy))   # check a variable's type
accuracy_str = str(accuracy)   # type casting
```
Hint: Python doesn't need you to declare a type upfront — the variable's type is decided by what you assign it. Worth naming the core types once: `int`, `float`, `str`, `bool`, `None`.
...
# Comparison
print(accuracy > 90)        # True/False

# Logical
print(is_trained and accuracy > 90)

# Assignment shorthand
epochs += 10                # same as epochs = epochs + 10
```
Hint: group them as arithmetic / comparison / logical / assignment — most audiences have seen these before, this is a fast recap, not new material.

---

## 2. Control Structures (15 min)
*Optional / audience-dependent — same call as above.*

### 2.1 if / elif / else (5 min)
```python
if accuracy >= 90:
    print("Excellent model")
elif accuracy >= 75:
    print("Good, needs tuning")
else:
    print("Needs more work")
```
Hint: standard branching — condition, one true path taken.
...
for i in range(3):          # loop a fixed number of times
    print(i)
```
Hint: "loop over a collection" is the pattern he'll use constantly once looping over API results, chunks of text, or batches of data.

### 2.3 while loops + break/continue (5 min)
```python
epochs_left = 3
while epochs_left > 0:
    print(f"{epochs_left} epochs left")
    epochs_left -= 1
...
for score in scores:
    ...
    print(score)
```
Hint: `while` = "keep going until a condition is false" (vs `for` = "go through a known collection/range"). `break`/`continue` are quick add-ons, not worth dwelling on.

---

## 3. Data Structures in Python (15 min)
...
## 4. OOP Concepts — Overview (20 min)
...
## 6. Exception Handling (10 min)

### 6.1 try/except basics (5 min)
```python
try:
    result = 10 / 0
...
## 7. Live Demo: Calling the OpenAI API and Reading the Response (3–4 min)
